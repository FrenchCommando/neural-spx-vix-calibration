...
class BrownianMotion(torch.nn.Module):
    ...
    def f(self, t, x):
        b = torch.zeros_like(x, device="cuda:0" if torch.cuda.is_available() else "cpu")
        return b

    def g(self, t, x):
        N = x.shape[0]
        ones = torch.ones([N, 1], device="cuda:0" if torch.cuda.is_available() else "cpu")
        zeros = torch.zeros([N, 1], device="cuda:0" if torch.cuda.is_available() else "cpu")
        V = torch.concat([ones, zeros, ones, zeros], axis=-1)
        V = V.reshape([N, 2, 2])
        return V

    def forward(self, t, bm):
        xy0 = torch.zeros(bm.shape, device="cuda:0" if torch.cuda.is_available() else "cpu")
        dt = bm.dt
        with torch.no_grad():
...
class GeneratorIto(torch.nn.Module):
    ...
    def f_and_g(self, t, XY):
        N = XY.shape[0]
        XY = XY[:, :2]
        tXY = torch.cat([t * torch.ones([N, 1], device="cuda:0" if torch.cuda.is_available() else "cpu"), XY], axis=1)
    ...
    def forward(self, t, xy0, bm=None):
        ...
        xyr0 = torch.concat(
            [xy0, torch.zeros([batch_size, 1], device="cuda:0" if torch.cuda.is_available() else "cpu")], axis=-1
        )
        if bm is None:
            t_start = t[0]
            t_end = t[-1]
            ...
            bm = torchsde.BrownianInterval(
                t0=t_start, t1=t_end, dt=self.dt, size=[batch_size, 2], device="cuda:0" if torch.cuda.is_available() else "cpu",
            )
